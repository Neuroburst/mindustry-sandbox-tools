...
function currentunit(){
	...
	if (diag.statsTable() != null){diag.updatestats(usfilter, diag.statsTable(), unitstat)};
};
...
Events.on(EventType.WorldLoadEvent, e => {
	...
	// if (diag.rulesTable() != null){diag.createRulesDialog(true); diag.updatestats(rfilter, diag.rulesTable(), Vars.state.rules)};
	bbteamRect.tint.set(Vars.player.team().color);
});
...
module.exports = {
	...
	spawningLabelText : spawningLabelText,
	getviewAirRange : () => viewAirRange,
	setviewAirRange : (v) => viewAirRange = v,
	getviewGroundRange : () => viewGroundRange,
	setviewGroundRange : (v) => viewGroundRange = v,
	fillMode : fillMode,
	spawning : spawning,
